...
const components: JSXMapSerializer = {
  ...
  heading3: ({ children }) => (
    <Heading
      as='h3'
      size='sm'
      className='mb-12 text-center font-medium sm:text-left'
    >
      {children}
    </Heading>
  ),
  paragraph: ({ children }) => (
    <p className=' mb-12 text-center font-body text-base font-medium text-slate-600 sm:text-left'>
      {children}
    </p>
  )
};
...
const Features = ({ slice }: FeaturesProps): JSX.Element => {
  return (
    ...
    >
      ...
      <div className='mx-auto max-w-5xl grid-cols-2 place-items-center gap-x-8 gap-y-12 sm:grid sm:place-items-start lg:grid-cols-4 '>
        {slice.items.map((item, index) => (
          <div
            key={index}
            className='grid max-w-xs place-items-center sm:place-items-start'
          >
            {/* <>{item.icon}</> */}
